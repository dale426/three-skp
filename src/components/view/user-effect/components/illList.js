export const illDesList = {
    fsmyk: [{
        title: '系统性红斑狼疮',
        position: '全身',
        carame: [-12, 18, 18],
        description: ['《EULAR/ERA-EDTA 成人及儿童狼疮性肾炎的管理建议（2012版）》', '《系统性红斑狼疮诊断及治疗指南（2010版）》']
    }, {
        title: '狼疮性肾炎',
        carame: [0, 12, 12],
        position: '肾脏',
        description: ['《KDIGO肾小球肾炎的临床实践指南（2012版）》', '《EULAR/ERA-EDTA 成人及儿童狼疮性肾炎的管理建议（2012版）》']
    }, {
        title: '炎性肌病合并肺间质病变',
        carame: [0, 14, 14],
        position: '全身及肺部',
        description: ['《美国胸科协会 CTD-ILD治疗指南（2013版）》']
    }, {
        carame: [5, 18, 18],
        title: '原发性中枢神经系统血管炎',
        position: '全身血管',
        description: ['《原发性中枢神经系统血管炎诊断与治疗中国专家共识(2017版)》']
    }, {
        title: 'ANCA相关性血管炎',
        carame: [0, 18, 18],
        position: '全身血管',
        description: ['《EULAR／ERA-EDTA建议：ANCA相关性血管炎的管理(2016版)》']
    }],
    xhk: [{
        title: '自身免疫性肝炎',
        carame: [0, 12, 12],
        position: '肝脏',
        description: ['《2015版EASL临床实践指南：自身免疫性肝炎》']
    }],
    xyk: [{
        title: '难治性血细胞减少症',
        carame: [0, 16, 16],
        position: '全身血管',
        description: []
    }, {
        title: '骨髓移植',
        carame: [0, 20, 20],
        position: '全身血管',
        description: ['《EBMT-ELN规范化操作建议移植物抗宿主病(GVHD)的预防和管理（2014版）》']
    }],
    sjnk: [{
        title: '视神经脊髓炎',
        carame: [0, 12, 12],
        position: '眼睛',
        description: ['《中国视神经脊髓炎谱系疾病诊断与治疗指南（2016版）》']
    }, {
        title: '多发性硬化',
        position: '全身肌肉',
        carame: [-10, 16, 16],
        description: ['《多发性硬化诊断和治疗中国专家共识 (2011版 )》']
    }, {
        title: '自身免疫性脑炎',
        position: '脑',
        carame: [0, 12, 12],
        description: ['《中国中国自身免疫性脑炎诊治专家共识（2017版）》']
    }, {
        title: '重症肌无力',
        position: '全身肌肉',
        carame: [16, 16, 16],
        description: ['《中国重症肌无力诊断和治疗指南（2015版）》', '《ABN管理指南：重症肌无力（2015版）》']
    }],
    snk: [{
        title: '难治性肾病综合征',
        carame: [0, 12, 12],
        position: '肾脏',
        description: ['《KDIGO肾小球肾炎的临床实践指南（2012版）》', '《中国成人肾病综合征免疫抑制治疗专家共识（2014版）》']
    }, {
        title: '系膜增生型IgA肾病',
        position: '肾脏',
        carame: [0, 12, 12],
        description: []
    }],
    pfk: [{
        title: '皮肤型红斑狼疮',
        carame: [0, 16, 16],
        position: '全身皮肤',
        description: ['《皮肤型红斑狼疮诊疗指南(2012版)》', '《 EDF/EADV指南：皮肤型红斑狼疮的治疗(2016版)》']
    }, {
        title: '天疱疮',
        position: '全身皮肤',
        carame: [0, 17, 17],
        description: ['《EDF/EADV 大疱性天疱疮管理指南（2015 版）》']
    }]
}
